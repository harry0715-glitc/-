...
test('admin photo lookup falls back to Google only for a migrated legacy photo', { concurrency: false }, async () => {
  setAdminConfig();
  process.env.SUPABASE_URL = 'https://example.supabase.co';
  process.env.SUPABASE_SERVICE_ROLE_KEY = 'server-key-for-test-only';
  process.env.SUPABASE_DATA_MODE = 'supabase';
  ...
  globalThis.fetch = async (url, options = {}) => {
    const text = String(url);
    const body = text === APPS_SCRIPT_URL && options.body ? JSON.parse(options.body) : null;
    if (text === APPS_SCRIPT_URL) {
      gasActions.push(body.action);
      if (body.action === 'adminLogin') {
      ...
      if (body.action === 'adminGetPhoto') {
        return new Response(JSON.stringify({ ok: true, data: { dataUrl: photoDataUrl } }), { status: 200 });
      }
    }
    if (text.includes('/rest/v1/managers')) {
      return new Response(JSON.stringify([managerRow]), { status: options.method === 'POST' ? 201 : 200 });
    }
    ...
    if (text.includes('/rest/v1/audit_logs')) return new Response('[]', { status: 201 });
    throw new Error(`Unexpected request: ${text}`);
  };
  ...
  const response = await adminApi(request('/api/admin', {
    ...
    payload: { id: 'worker-legacy' },
  }, { Cookie: loggedIn.cookie }));
  const body = await response.json();

  assert.equal(response.status, 200);
  assert.equal(body.data.dataUrl, photoDataUrl);
  assert.deepEqual(gasActions, ['adminLogin', 'adminGetPhoto']);
});
